...
// create
    ...
    {
        echo '<i class="cegg-disclaimer" title="' . \esc_attr(self::getAmazonDisclaimer()) . '">&#9432;</i>';
    }
    ...
    public static function hideParamPrepare($hide)
    {
        ...
        $allowed_hide = array('price', 'priceOld', 'domain', 'rating', 'title', 'stock_status', 'img', 'merchant', 'description');
    ...
    public static function getImageSizesRatio(array $item, $max_width, $max_height)
    {
        if ($item['module_id'] == 'Amazon' && strpos($item['img'], 'https://m.media-amazon.com') !== false)
        {
            ...
            //\ContentEgg\prn('$max_width: ' . $max_width);
            //\ContentEgg\prn('$max_height:' . $max_height);
            

            //\ContentEgg\prn('$width: ' . $width);
            //\ContentEgg\prn('$height:' . $height);

            $ratio = $width / $height;
            //\ContentEgg\prn('$ratio:' . $ratio);
            //\ContentEgg\prn($ratio);
