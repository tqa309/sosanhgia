...
class KelkooModule extends AffiliateParserModule {
    ...
    private function fillMerchantInfo($content)
    {
        ...
        if (!isset($this->merchants[$merchant_id]))
        {
            ...
            elseif (isset($result['Merchant']))
                $this->merchants[$merchant_id] = $result['Merchant'];
            else
                return;
        }

        if (isset($this->merchants[$merchant_id]['merchantUrl']))
            $merchantUrl = $this->merchants[$merchant_id]['merchantUrl'];
        elseif (isset($this->merchants[$merchant_id]['MerchantUrl']))
            $merchantUrl = $this->merchants[$merchant_id]['MerchantUrl'];
        else
            $merchantUrl = '';

        if (isset($this->merchants[$merchant_id]['profile']['logo']['url']))
            $logo = $this->merchants[$merchant_id]['profile']['logo']['url'];
        elseif (isset($this->merchants[$merchant_id]['Profile']['Logo']['Url']))
            $logo = $this->merchants[$merchant_id]['Profile']['Logo']['Url'];
        else
            $logo = '';

        if ($merchantUrl)
            $content->domain = TextHelper::getHostName($merchantUrl);
